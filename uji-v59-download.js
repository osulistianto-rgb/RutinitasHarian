...
/* Empat modul unik tertanam langsung. Kantor dan Pribadi dibentuk dari
   satu mesin pekerjaan dengan konfigurasi berbeda. */
for (const m of indeks.matchAll(/(\w+):decode\('([A-Za-z0-9+/=]{200,})'\)/g)) {
  modul[m[1]] = Buffer.from(m[2], 'base64').toString('utf8');
}

const dasarMatch = indeks.match(
  /const PEKERJAAN_DASAR=decode\('([A-Za-z0-9+/=]{200,})'\);/
);
const konfigurasiMatch = indeks.match(
  /^const konfigurasiPekerjaan=(\{.*\});$/m
);

if (!dasarMatch || !konfigurasiMatch) {
  throw new Error('Mesin pekerjaan terpadu atau konfigurasinya tidak ditemukan.');
}

const DASAR_PEKERJAAN = Buffer.from(dasarMatch[1], 'base64').toString('utf8');
const KONFIGURASI_PEKERJAAN = JSON.parse(konfigurasiMatch[1]);

function bentukPekerjaan(konteks) {
  const c = KONFIGURASI_PEKERJAAN[konteks];
  if (!c) throw new Error('Konfigurasi pekerjaan tidak ditemukan: ' + konteks);

  let html = DASAR_PEKERJAAN;
  const nilai = {
    '__RP_DOK_TITLE__': c.dokTitle,
    '__RP_ACCENT__': c.accent,
    '__RP_TINT__': c.tint,
    '__RP_H1__': c.h1,
    '__RP_KONTEKS__': c.konteks,
    '__RP_KAT_AWAL__': JSON.stringify(c.katAwal),
    '__RP_KALENDER_SIGN__': c.kalenderSign,
    '__RP_KALENDER_CAT__': c.kalenderCat,
    '__RP_PDF_TITLE__': c.pdfTitle,
    '__RP_PDF_FILENAME__': c.pdfFilename
  };

  for (const [tanda, isi] of Object.entries(nilai)) {
    html = html.split(tanda).join(String(isi));
  }
  return html;
}

modul.kantor = bentukPekerjaan('kantor');
modul.pribadi = bentukPekerjaan('pribadi');

const URUTAN_MODUL = ['beranda', 'routine', 'report', 'kantor', 'pribadi', 'note'];
const NAMA = URUTAN_MODUL.filter(n => Object.prototype.hasOwnProperty.call(modul, n));
...
async function jalankan() {
  ...
  periksa('enam modul logis terbentuk', NAMA.length === 6,
    'ditemukan ' + NAMA.length + ': ' + NAMA.join(', '));
  periksa('Kantor dan Pribadi memakai satu mesin',
    !!dasarMatch && indeks.includes("kantor:buatPekerjaan('kantor')") &&
    indeks.includes("pribadi:buatPekerjaan('pribadi')"));
  periksa('data Kantor tetap terpisah',
    modul.kantor.includes("const KONTEKS='kantor'") &&
    modul.kantor.includes("const KUNCI='kerja:'+KONTEKS"));
  periksa('data Pribadi tetap terpisah',
    modul.pribadi.includes("const KONTEKS='pribadi'") &&
    modul.pribadi.includes("const KUNCI='kerja:'+KONTEKS"));
